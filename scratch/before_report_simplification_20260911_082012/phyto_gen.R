...
for (sf in src_files) {
  ...
  age_keep <- select_age(dat[["age_ka_bp"]], source_rows)
  ...
  add_issue <- function(field, rows, problem) {
    report_issue(field, source_rows[rows], dat[[fields[[field]]]][rows], problem)
  }
  if ("SST_other_degC" %in% names(dat)) {
    other <- which(is.na(as_chr(src_col("sst"))) & !is.na(as_chr(dat$SST_other_degC)))
    report_action("SST_other_degC", source_rows[other], dat$SST_other_degC[other],
                 "Used alternate SST with its matching uncertainty and method")
    sst_cols <- c(SST_degC="SST_other_degC",
                  SST_degC_uncertainty_pos="SST_other_degC_uncertainty_pos",
                  SST_degC_uncertainty_neg="SST_other_degC_uncertainty_neg",
                  SST_degC_uncertainty_type="SST_other_degC_uncertainty_type",
                  SST_degC_source="SST_other_name")
    if (length(other)) for (co in names(sst_cols)) {
      alt <- sst_cols[[co]]
      if (co == "SST_degC_source") {
        if (alt %in% names(dat)) {
          use <- other[!is.na(as_chr(dat[[alt]][other]))]
          dat[[co]][use] <- dat[[alt]][use]
        }
      } else {
        dat[[co]][other] <- if (alt %in% names(dat)) dat[[alt]][other] else NA
      }
    }
  }
  for (field in numeric_fields) {
    ...
    report_action(field, source_rows[note], dat[[fields[[field]]]][note], "Numeric value read before note; original text retained here")
    raw[note] <- sub(" +note:.*", "", raw[note])
    bad <- which(!is.na(raw) & is.na(as_num(raw)))
    add_issue(field, bad, "Non-numeric value; generation stopped")
  ...
  check_output(out_df, source_rows, c("F", "G", "H", "I", "J", "K", LETTERS[14:21], LETTERS[23:26], "AB", "AC", "AD", "AE", "AF"), c("N", "R", "W", "AB", "AF"))
...
if (!length(metadata)) {
  write.csv(data.frame(file=character(), source_row=integer()),
            file.path(out_dir, "phyto_record_metadata.csv"), row.names=FALSE)
  write.csv(data.frame(file_1=character(), source_row_1=integer(), file_2=character(), source_row_2=integer()),
            file.path(out_dir, "phyto_duplicate_candidates.csv"), row.names=FALSE)
  write.csv(link_report(data.frame(unique_id=character(), parent_id=character(), child_id=character())),
            file.path(out_dir, "phyto_parent_child_links.csv"), row.names=FALSE)
}
