...
report_action <- function(field, rows, value, problem) {
  if (!length(rows)) return(invisible(NULL))
  actions <<- unique(rbind(actions, data.frame(file=basename(sf), source_row=rows,
                                              field=field, value=as.character(value), problem=problem)))
}
...
finish_reports <- function() {
  processing_log <- actions
  names(processing_log)[names(processing_log) == "problem"] <- "action"
  write.csv(processing_log, file.path(out_dir, paste0(proxy, "_processing_log.csv")), row.names=FALSE, na="")
  write.csv(problems, file.path(out_dir, paste0(proxy, "_data_issues.csv")), row.names=FALSE, na="")
  write.csv(age_screen, file.path(out_dir, paste0(proxy, "_age_screen.csv")), row.names=FALSE, na="")
  for (output in unique(unname(study_outputs))) {
    files <- names(study_outputs)[study_outputs == output]
    dest <- sub("\\.xlsx$", "_data_issues.csv", output)
    write.csv(problems[problems$file %in% files, ], dest, row.names=FALSE, na="")
  }
