...
for (sf in src_files) {
  ...
  if (length(hit) != 1) {
    report_issue("sheet", NA_integer_, paste(sheets, collapse="; "),
                 "Boron data sheet missing or ambiguous; generation stopped")
    stop("Check boron isotope sheet in: ", sf)
  }
  source_sheet_i <- sheets[hit]
  ...
  if (!identical(names(dat)[33], "Age.(ka)")) {
    report_issue("age header", NA_integer_, names(dat)[33], "Unexpected age column; generation stopped")
    stop("Check age column in: ", sf)
  }
  age_keep <- select_age(dat[[33]], source_rows)
  ...
  out_df <- data.frame(
    ...
    F = colF, G = colG, H = as_num(colH)/1000, I = colI/1000, J = colJ/1000,
  ...
  report_issue("S", source_rows[flagS], CB[flagS], "Alternative source value present but Mg/Ca column S remains blank")
  for (spec in list(c(34, 35, 36), c(77, 78, 79))) {
    check_uncertainty(dat, spec[1], spec[-1],
