...
uncertainty <- function(mean, pos, neg, type, field, bounds=TRUE) {
  ...
  report_issue(field, source_rows[blank], rep(NA_character_, length(blank)), "Used explicit 2s magnitude headers; distribution label absent")
  if (!bounds) {
...
for (sf in src_files) {
  ...
  report_issue("AG", source_rows[age_only], dat[[33]][age_only], "Age-only row with no sample metadata or measurements; excluded")
  ...
  mg_unc <- uncertainty(mg, numeric_col("CE"), numeric_col("CF"), dat[[82]], "CD", FALSE)
  temp <- numeric_col("BX")
...
if (length(generated_data)) {
  ...
  for (file in names(generated_data)) {
    ...
    sf <- file
    report_issue("duplicate",m$source_row[removed],
  ...
  write.csv(m,file.path(out_dir,"boron_record_metadata.csv"),row.names=FALSE,na="")
} else {
  candidates <- data.frame(action=character(),file_1=character(),source_row_1=integer(),file_2=character(),source_row_2=integer())
  write.csv(data.frame(file=character(),source_row=integer()),file.path(out_dir,"boron_record_metadata.csv"),row.names=FALSE)
}
write.csv(candidates,file.path(out_dir,"boron_duplicate_candidates.csv"),row.names=FALSE,na="")
finish_reports()
