...
finish_reports <- function() {
  ...
  if (length(old)) {
    archive <- file.path("scratch", "previous_reports", proxy, format(Sys.time(), "%Y%m%d_%H%M%OS6"))
    dir.create(archive, recursive=TRUE)
    if (!all(file.rename(old, file.path(archive, basename(old))))) stop("Could not archive old reports")
  }
